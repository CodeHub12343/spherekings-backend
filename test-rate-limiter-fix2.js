...
async function testAuth() {
  try {
    ...
    const token = loginRes.data.tokens?.accessToken || loginRes.data.data?.accessToken || loginRes.data.accessToken;
    if (!token) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (meRes.status === 200) {
        ...
        console.log('   Response:', JSON.stringify(meRes.data).substring(0, 100));
      }
    }
  } catch (error) {
